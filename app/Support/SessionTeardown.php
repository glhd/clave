...
class SessionTeardown
{
	...
	protected function suspendOrDeleteVm(SessionContext $context): void
	{
		...
		if ($context->ephemeral) {
			$this->checklist('Stopping ephemeral VM...')
				->run(fn() => $this->tart->stop($context->vm_name));
			$this->checklist('Deleting ephemeral VM...')
				->run(fn() => $this->tart->delete($context->vm_name));
		} else {
			$this->checklist('Suspending VM...')
				->run(fn() => $this->tart->suspend($context->vm_name));
		}
	}
